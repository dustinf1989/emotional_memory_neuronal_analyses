function explVar = dpca_explainedVariance(Xfull, W, V, time, events, varargin)
...
explVar.individualPCA = (S.^2'/ explVar.totalVar * 100);
...
% for i=1:9
%     nexttile()
%     plot(time, X1(1:4:end,i), 'color', rgb('FireBrick'))
%     hold on;
%     plot(time, X1(2:4:end,i), '--', 'color', rgb('FireBrick'))
%     plot(time, X1(3:4:end,i), 'color', rgb('Navy'))
%     plot(time, X1(4:4:end,i), '--', 'color',  rgb('Navy'))
%     xline(events(1));
%     xline(events(2));
...
% f1 = figure(); % Great way to keep figures invisible!
myFig = figure('Visible','On','Units','centimeters', 'Position', [0 0 19.5 4]);
% set(f1, 'PaperUnits', 'inches', 'PaperType', 'A4', 'PaperPositionMode', 'auto','units','normalized','outerposition',[0 0 1 1],'RendererMode','manual','Renderer','painters');
tiledlayout(1,5)
for i=1:5 % outsmall = {'RHit','Miss','CR','KHit'};
    nexttile()
    plot(time, U1(1:4:end,i), 'color', rgb('FireBrick'))
    hold on;
    plot(time, U1(2:4:end,i), '--', 'color', rgb('FireBrick'))
    plot(time, U1(3:4:end,i), 'color', rgb('Navy'))
    plot(time, U1(4:4:end,i), '--', 'color',  rgb('Navy'))
    title(sprintf('PC %i [%0.1f%%]',i,explVar.individualPCA(i)),'FontWeight','normal')
    xlim([-0.2, 1.5]);
    xline(events(1));
    xline(events(2));
%     plot([events; events], yspan, 'Color', [0.6 0.6 0.6])
...
%         ylabel('Normalized Firing Rate')
    elseif i == 1
        ylabel(sprintf('%i neurons\nNormalized Firing Rate',size(Xfull,1)))
    end
end
% legend({'eRHit','eMiss','nRHit','nMiss'})
