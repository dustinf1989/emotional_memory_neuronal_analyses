...
tiledlayout(1,5) % First 5 PCs
for i=1:5 % outsmall = {'RHit','Miss','CR','KHit'};
    nexttile()
    if i == 1 || i ==2
        plot(time, -U1(1:6:end,i), 'color', rgb('FireBrick'))
        hold on;
        plot(time, -U1(2:6:end,i), '--', 'color', rgb('FireBrick'))
        plot(time, -U1(3:6:end,i), ':', 'color', rgb('FireBrick'))
        plot(time, -U1(4:6:end,i), 'color', rgb('Navy'))
        plot(time, -U1(5:6:end,i), '--', 'color',  rgb('Navy'))
        plot(time, -U1(6:6:end,i), ':', 'color', rgb('Navy'))
    else
        plot(time, U1(1:6:end,i), 'color', rgb('FireBrick'))
        hold on;
        plot(time, U1(2:6:end,i), '--', 'color', rgb('FireBrick'))
...
% legend({'eRHit','eMiss','eCR','nRHit','nMiss','nCR'})
