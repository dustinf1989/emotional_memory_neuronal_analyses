...
%         outs = {'CorrRem','CorrFam','Missed','CorrNotRem','FalseRem','FalseFam'};
        outs = {'Remembered','Forgotten','Know'};
        styles = {'-','--',':'};
        for f = 1:numOfStimuli
%         f = 1; %
            plot([0.5 1], [f*2 f*2], 'LineStyle', styles(f), 'color', [0.5 0.5 0.5], 'LineWidth', 1.5)
            text(1.2, f*2, outs{f})
        end
...
%         end
        ...
        plot([0.5 1], [-2 -2], 'Color',rgb('FireBrick'), 'LineWidth', 1.5)
        plot([0.5 1], [-4 -4], 'Color',rgb('Navy'), 'LineWidth', 1.5)
...
%     colors = lines(numOfStimuli);
    ...
    if compNum == 2 || compNum ==3  % Reverse direction so emotional is on top of neutral % Was only 2 for emotional
        for f=1:numOfDecisions 
            ...
            plot(time, -squeeze(data(1, 3, f, :)), ':', 'color', colors(f,:), 'LineWidth', 1.25)
        end
    else
        for f=1:numOfDecisions 
            plot(time, squeeze(data(1, 1, f, :)), 'color', colors(f,:), 'LineWidth', 1.25)
