...
if ntrial==1
  ...
  endsmp = nan(ntrial, 1);
  for i=1:ntrial
%     begsmp(i) = nearest(time, data.time{i}(1)); % DF removed these lines because they seemed pointless
%     endsmp(i) = nearest(time, data.time{i}(end));
    begsmp(i) = 1; % nearest(time, data.time{i}(1));
    endsmp(i) = length(data.time{i}); %nearest(time, data.time{i}(end));
    tmptrial(i,:,begsmp(i):endsmp(i)) = data.trial{i};
%     if i > 46 % DF: Patient1 macro electrode issue
