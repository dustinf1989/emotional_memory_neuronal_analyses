...
%
% This script assumes both cfg.trl and spike.timestamp are in time units (seconds) not sample indicies.
%
...
%     end
    ...
    for j=1:size(trl,1)
      ...
      for k=1:length(sel)
        ...
        indx_s = find(data.time{j}(end,:) == indx); % DF: Added this code so I could use seconds instead of timestamps (in samples)
        data.trial{j}(end,indx_s) = data.trial{j}(end,indx_s)+1;
      end % for k
    end % for j
  end % for i
