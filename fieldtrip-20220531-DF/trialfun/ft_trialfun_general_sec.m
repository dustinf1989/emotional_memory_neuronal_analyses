function [trl, event] = ft_trialfun_general_sec(cfg)
...
%       trloff = round(-cfg.trialdef.prestim); % DF: Why was this rounding a decmiel?
      trloff = -cfg.trialdef.prestim; % DF: Why was this rounding a decmiel? Times in seconds with decimels
      % also shift the begin sample with the specified amount
      trlbeg = event(i).timestamp + trloff;
    end
...
%       trldur = round((cfg.trialdef.poststim+cfg.trialdef.prestim)*hdr.Fs) - 1; % DF: Remove *Fs, why was this rounded anyway? Remove -1, not sure why it was here?
      trldur = (cfg.trialdef.poststim+cfg.trialdef.prestim);
    end
