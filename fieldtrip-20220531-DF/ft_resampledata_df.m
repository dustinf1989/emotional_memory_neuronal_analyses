...
if usefsample
  ...
  for itr = 1:ntr
    begsample(itr) = round(cfg.origfs * round(data.time{itr}(1))); % DF added round() to make sure number of samples match - so stupid to have to do this
    endsample(itr) = round(cfg.origfs * round(data.time{itr}(end)));
  end
  ...
  if any(strcmp(cfg.method, {'resample','decimate', 'downsample', 'mean', 'median'})) % DF added resample here
